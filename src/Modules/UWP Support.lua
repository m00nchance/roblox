--[[
	...
	Universal ESP Module by m00nchance © CC0 1.0 Universal (2023)
	https://github.com/m00nchance
...
if m00nchanceDeveloperESP then
	m00nchanceDeveloperESP:Exit()
end

--// Settings

getgenv().m00nchanceDeveloperESP = {
	DeveloperSettings = {
		Path = "m00nchance Developer/m00nchance ESP/Configuration.cfg",
...
local Environment = getgenv().m00nchanceDeveloperESP
...
local CoreFunctions = {
	...
	GenerateHash = function(Bits)
		...
		for _ = 1, Bits do
			Result ..= ("m00nchance_ESP")[mathrandom(1, 2) == 1 and "upper" or "lower"](stringchar(mathrandom(97, 122)))
		end
...
local UtilityFunctions = {
	...
	WrapObject = function(self, Object, PseudoName, Allowed, RenderDistance)
		assert(self, "m00nchance_ESP > UtilityFunctions.WrapObject - Internal error, unassigned parameter \"self\".")
		...
		if not Entry.IsAPlayer then
			if not pcall(function()
				...
				warn("m00nchance_ESP > UtilityFunctions.WrapObject - Attempted to wrap object of an unsupported class type: \""..(__index(Entry.Object, "ClassName") or "N / A").."\"")
...
Environment.UnwrapAll = function(self) -- METHOD | (<void>) => <void>
	assert(self, "m00nchance_ESP.UnwrapAll: Missing parameter #1 \"self\" <table>.")
...
Environment.Restart = function(self) -- METHOD | (<void>) => <void>
	assert(self, "m00nchance_ESP.Restart: Missing parameter #1 \"self\" <table>.")
...
Environment.Exit = function(self) -- METHOD | (<void>) => <void>
	assert(self, "m00nchance_ESP.Exit: Missing parameter #1 \"self\" <table>.")
	...
	if self:UnwrapAll() then
		...
		for Index, _ in next, Environment do
			getgenv().m00nchanceDeveloperESP[Index] = nil
		end

		LoadESP = nil; Recursive = nil; Loaded = false

		getgenv().m00nchanceDeveloperESP = nil
	end
end
...
Environment.UpdateConfiguration = function(DeveloperSettings, Settings, Properties) -- (<table> DeveloperSettings, <table> Settings, <table> Properties) => <table> New Environment
	assert(DeveloperSettings, "m00nchance_ESP.UpdateConfiguration: Missing parameter #1 \"DeveloperSettings\" <table>.")
	assert(Settings, "m00nchance_ESP.UpdateConfiguration: Missing parameter #2 \"Settings\" <table>.")
	assert(Properties, "m00nchance_ESP.UpdateConfiguration: Missing parameter #3 \"Properties\" <table>.")

	getgenv().m00nchanceDeveloperESP.DeveloperSettings = DeveloperSettings
	getgenv().m00nchanceDeveloperESP.Settings = Settings
	getgenv().m00nchanceDeveloperESP.Properties = Properties

	Environment = getgenv().m00nchanceDeveloperESP

	return Environment
end

Environment.LoadConfiguration = function(self) -- METHOD | (<void>) => <void>
	assert(self, "m00nchance_ESP.LoadConfiguration: Missing parameter #1 \"self\" <table>.")
...
Environment.SaveConfiguration = function(self) -- METHOD | (<void>) => <void>
	assert(self, "m00nchance_ESP.SaveConfiguration: Missing parameter #1 \"self\" <table>.")
